...
const createWorkSchedule = async ({ date, job_name, start_time, location, note, created_by, assigned_users = [] }) => {
    ...
    try {
        ...
        // Gán lịch cho các user được chọn
        if (assigned_users.length > 0) {
            const registrationPromises = assigned_users.map(userId => {
                return client.query(
                    `INSERT INTO work_schedule_registration (schedule_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
                    [newSchedule.id, userId]
                );
            });
